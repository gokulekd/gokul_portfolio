...
import 'featured_projects_section.dart';

class ProjectsHeroSection extends StatefulWidget {
  const ProjectsHeroSection({super.key});

  @override
  State<ProjectsHeroSection> createState() => _ProjectsHeroSectionState();
}

class _ProjectsHeroSectionState extends State<ProjectsHeroSection>
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 12,
                  children: [
                    StatPill(label: '25+ Projects', icon: Icons.folder_outlined),
                    StatPill(label: 'Mobile & Web', icon: Icons.devices_outlined),
                    StatPill(label: '2024 – Present', icon: Icons.calendar_today_outlined),
                  ],
                ),
              ],
            ),
...
class StatPill extends StatelessWidget {
  ...
  const StatPill({super.key, required this.label, required this.icon});
...
class FeaturedProjectsSectionWrapper extends ConsumerWidget {
  const FeaturedProjectsSectionWrapper({super.key});
                ...
                builder: (context, constraints) {
                  ...
                  return Wrap(
                    ...
                    children: filtered.map((project) => SizedBox(
                      width: cardWidth,
                      child: AppProjectGridCard(project: project),
                    )).toList(),
                  );
                },
              ),
...
class AppProjectGridCard extends StatelessWidget {
  final AppProject project;

  const AppProjectGridCard({super.key, required this.project});
          ...
          if (project.appBannerUrl.isNotEmpty)
            Image.network(
              ...
              errorBuilder: (_, __, ___) =>
                  ImagePlaceholder(colorScheme: colorScheme),
            )
          else
            ImagePlaceholder(colorScheme: colorScheme),
...
class ProjectsCTASection extends ConsumerWidget {
  const ProjectsCTASection({super.key});
...
class ImagePlaceholder extends StatelessWidget {
  ...
  const ImagePlaceholder({super.key, required this.colorScheme});
...
class ActionBtn extends StatelessWidget {
  ...
  const ActionBtn({super.key, required this.icon,
    required this.label,
    required this.onTap,
