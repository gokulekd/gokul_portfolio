...
class SubmissionsWorkspace extends ConsumerWidget {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              if (note.isNotEmpty) {
                ref.read(adminPortalProvider.notifier).addSubmissionNote(note);
              }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final subs = state.liveSubmissions;
    final selectedSub = state.selectedSubmission;
                  ...
                  .map(
                    (sub) => Padding(
                      ...
                      child: SubmissionCard(
                        ...
                        onTap: () => notifier.selectSubmission(sub),
                      ),
                    ),
                  )
                  .toList(),
          ...
          : Column(
              ...
              children: [
                AdminSectionHeader(
                  ...
                  description: selectedSub.company,
                ),
                const SizedBox(height: 18),
                PreviewTile(
                  title: 'Contact',
                ...
                PreviewTile(
                  title: 'Status',
                  value: selectedSub.statusLabel,
                  icon: Icons.flag_rounded,
                  color: const Color(0xFFFFB44C),
                ),
                if (selectedSub.notes.isNotEmpty) ...[
                  const SizedBox(height: 12),
                  _NotesSection(notes: selectedSub.notes),
                ],
                const SizedBox(height: 18),
                Wrap(
                  spacing: 12,
                  ...
                  children: [
                    AdminPrimaryButton(
                      ...
                      onPressed: selectedSub.status == SubmissionStatus.inProgress
                          ? null
                          : () => notifier.markSubmissionInProgress(),
                    ),
                    AdminGhostButton(
                      label: 'Add note',
